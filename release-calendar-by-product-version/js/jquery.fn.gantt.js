...
var dayx = new Date();
                dayx.setDate(dayx.getDate() - 365);
                element.dateStart = new Date(dayx.getFullYear(), dayx
                    .getMonth(), dayx.getDate());

                var todayx = new Date();
                todayx.setDate(todayx.getDate() + 366);
                element.dateEnd = new Date(todayx.getFullYear(), todayx
                    .getMonth(), todayx.getDate());
   ...
   var $dataPanelBackground = $rightPanel.find(".dataPanelBackground");
               ...
               if (settings.scrollToToday) {
                    core.navigateTo(element, 'custom');
                    core.scrollPanel(element, 10);
...
if(entry.values[0].lable==="Not Updated")
{
 entries.push(
...
'     <rect width="50" height="1000" style="fill:rgb(0,0,0);fill-opacity:0" />'+
'     <g class="weekendBar"><rect x="50" width="20" height="1000" style="fill:rgb(255,255,255);" />'+
'     <path d="M 50.25 0 L 50.25 1000 M 69.75 0 L 69.75 1000" style="stroke:rgb(255,255,255);stroke-width: 0.5" /></g>'+
'  </pattern>'+
' <g class="ganttBackground" transform="scale(2.4 1)">'+
'  <rect width="100000" height="10000" fill="url(#bck)" />'+
'  <rect class="todayBar" x="20" width="30" height="10000" style="fill:rgb(255,255,102")" />'+
'  </g>'+
'</svg>');
...
var $scollPanels = $dataPanel.add($rightPanel.find(".dataPanelBackground"));
                ...
                case "end":
                   ...
                   $scollPanels.animate({ "margin-left": "-" + mLeft }, "fast", shift);
                    break;
                case "now":
                    ...
                    curMarg =$scollPanels.css("margin-left").replace("px", "");
                    val = $scollPanels.find(".today").offset().left - $scollPanels.offset().left;
                    val *= -1;
                    if (val > 0) {
                        val = 0;
                    } else if (val < maxLeft) {
                   ...
                   $scollPanels.animate({ "margin-left": val }, "fast", shift);
...
var $scrollPanels = $(".fn-gantt .dataPanel, .fn-gantt .dataPanelBackground",element);
                ...
                if ((e.pageX >= bPos.left) && (e.pageX <= bPos.left + bWidth)) {
                    ...
                    if (pPos >= 0) {
                        ...
                        $scrollPanels.css("margin-left", mLeft * -1);
                        element.scrollNavigation.panelMargin = mLeft * -1;
                    } else {
                        $scrollPanels.css("margin-left", pPos);
