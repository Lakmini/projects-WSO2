    ...
    $.fn.graphly = function(options) {  
        ...
        var methods = {
            ...
            drawLegend : function(ctx) {
                var x = globals.width - globals.paddingRight + 55;
              ...
              //  ctx.fillText('Legend', x, offset);
                ctx.font = "bold 11px \"lucida grande\",tahoma,verdana,arial,sans-serif";
...
ctx.arc(x, temp, 6, 0, 2 * Math.PI);
                    ctx.fillStyle = methods.getFillColor(i);
                    ctx.fill();
                    ctx.closePath();
                    ctx.textBaseline = 'top';
                    ctx.fillText(label, x + 100, temp - 1);
                });
...
if(totalSteps<10)
{
 var labelGap = chartHeight / totalSteps;
for(var i=0;i<=totalSteps;i++)
{
var value=(i-totalSteps)*-1;
 var dimensions = ctx.measureText(methods.roundToFixed(value, globals.decimalCount));
                    if (largestLabel == null || largestLabel < totalSteps) {
                        largestLabel = totalSteps;
                    }
 ...
 ctx.restore();
            ...
            drawLines : function(ctx) {
                ...
                $.each(settings.data.groups, function(i, group) {
                    ...
                    $.each(rectanglePoints, function(i, point) {
                        ctx.rect(point[0] - 5, point[1] - 5, 0, 0);
			ctx.arc(point[0] - 0,point[1] - 0 , 3, 0, 2 * Math.PI);
        ...
        return this.each(function() {
            ...
            if (settings.showLegend && settings.data.groups.length > 1) {
                globals.paddingRight += globals.largestLabelWidth + 70;
