...
$.post("/portal/controllers/apis/patching-trend-table/table.jag", {
    ...
    showLog("Please Select the  Product");

});

gadgets.HubSettings.onConnect = function () {
    gadgets.Hub.subscribe('listener', function (topic, data_from_publisher, subscriberData) {
        var pmt_product_name = data_from_publisher.msg[0].PMT_name;

        //alert(pmt_product_name);
        if (pmt_product_name != "") {
            $('.log').hide();
            drawTable(pmt_product_name);
        }
        else {
            showLog("No Data");
        }
...
function drawTable(selection) {
    ...
    for (var i in formated_data) {
        ...
        if (formated_data[i].product_name == selection) {

            dataSet[counter++] = formated_data[i];
        }
    }
    if (dataSet.length == 0) {
        showLog();
